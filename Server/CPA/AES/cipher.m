function [ciphertext,power_wave] = cipher (plaintext, w, s_box, poly_mat, vargin)
...
%  --------------- Amin 
power_wave = wgn(1,1500,0.01);
power_wave = power_wave/(max(power_wave));
hamming_table = HammingTable();

%  --------------- Amin
M = [0,15,54,57,83,93,101,106,149,0154,163,172,198,201,240,255];% Masks
offset = randi(16);
M = M([offset:16 1:offset-1]);
MaskOffset = reshape(M,4,4)'; 
MaskCompensate = bitxor(MaskOffset , mix_columns(shift_rows(MaskOffset),poly_mat));
MaskCompensate = MaskCompensate([2:16 1]);
...
% Amin
B = reshape(hamming_table(1+bitxor(state,A)),1,16)/6;
for i1 = 1:16
    for i2 = 1:16
        power_wave(100 + i1*16 + i2) = power_wave(100 + i1*16 + i2) + B(i2);
    end
end   
...
A = state;
    ...
    for k = 1:16
        state(k) = bitxor(s_box(1+bitxor(M(k),state(k))),M(mod(k,16)+1));
    end
...
for i1 = 1:16
    for i2 = 1:16
        power_wave(400 + i1*16 + i2) = power_wave(400 + i1*16 + i2) + B(i2);
    end
end   
break   
...
for i1 = 1:16
    for i2 = 1:16
        power_wave(700 + i1*16 + i2) = power_wave(700 + i1*16 + i2) + B(i2);
    end
end   
...
for i1 = 1:16
    for i2 = 1:16
        power_wave(1000 + i1*16 + i2) = power_wave(1000 + i1*16 + i2) + B(i2);
    end
end   
    
%Amin Mask Compensate
state = bitxor(MaskCompensate(),state);
